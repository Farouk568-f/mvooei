...
export const fetchStreamUrl = async (
    ...
    const cacheKey = `stream_cache_v4_${media_type}_${item.id}${season ? `_s${season}` : ''}${episode ? `_e${episode}` : ''}${specificProvider ? `_p_${specificProvider}` : '_p_auto'}${dubLang ? `_d_${dubLang}` : ''}`;
    try {
    ...
    if (specificProvider) {
        providersToTry = allProviders.filter(p => p.id === specificProvider);
    ...
    for (const provider of providersToTry) {
        try {
            ...
            if (typedResponse.status === 'success' && Array.isArray(typedResponse.links) && typedResponse.links.length > 0) {
                ...
                const PROXY_URL = (typeof window !== 'undefined' && location.hostname === 'localhost') ? '' : 'https://prox-q3zt.onrender.com';
                // Build proxied links for moviebox, then actively probe which ones are accepted by the proxy.
                let processedLinks = finalLinks.map(link => {
                    if (provider.id === 'moviebox') {
                        const prefix = PROXY_URL === '' ? '' : PROXY_URL;
                        return { ...link, url: `${prefix}/proxy?url=${encodeURIComponent(link.url)}` };
                    }
                    return link;
                });
...
export const downloadVideoToIndexedDB = async (
    ...
    const segmentSize = 2 * 1024 * 1024; // 2MB segments
    let url = initialUrl;

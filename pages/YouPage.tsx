...
const YouPage: React.FC = () => {
    ...
    const historyData = getScreenSpecificData('history', []);
    const favorites = getScreenSpecificData('favorites', []);
    const downloads = getScreenSpecificData('downloads', []);
    
    const lastWatched = historyData.length > 0 ? historyData[0] : null;
    const historyRest = historyData.length > 1 ? historyData.slice(1) : [];

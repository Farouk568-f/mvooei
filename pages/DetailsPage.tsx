...
import { fetchFromTMDB, fetchStreamUrl, downloadVideoToIndexedDB } from '../services/apiService';
import { Movie, Episode, Season } from '../types';
import { useProfile } from '../contexts/ProfileContext';
import { useTranslation } from '../contexts/LanguageContext';
import Layout from '../components/Layout';
import { IMAGE_BASE_URL, BACKDROP_SIZE, BACKDROP_SIZE_MEDIUM, POSTER_SIZE, AVAILABLE_PROVIDERS } from '../contexts/constants';
...
const SimilarItemCard: React.FC<{ item: Movie, index: number }> = ({ item, index }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="flex-shrink-0 w-32 cursor-pointer animate-fade-in-up"
      style={{ animationDelay: `${index * 50}ms` }}
    >
      <div className="relative overflow-hidden transition-all duration-300 ease-in-out transform rounded-lg shadow-lg bg-[var(--surface)] border-2 border-transparent interactive-card-sm">
        <img
...
const DetailsPage: React.FC = () => {
  ...
  const navigate = useNavigate();
  const { isFavorite, toggleFavorite, setToast, addDownload } = useProfile();
    ...
    const prefetchStream = async () => {
        if (item && type) {
            try {
                ...
                // Prefetch from the primary provider for faster loading on the player page.
                const primaryProvider = AVAILABLE_PROVIDERS[0]?.id;
                if (!primaryProvider) return;

                const data = await fetchStreamUrl(item, type, seasonToFetch, episodeToFetch, primaryProvider);
                if (data.links && data.links.length > 0) {
                    setPrefetchedStreamUrl(data.links[0].url);
                    console.log("Stream prefetched successfully from primary provider.");
                }
            } catch (error) {
  ...
  const handleDownload = async () => {
    if (!item) return;
    try {
      setToast({ message: 'Starting download...', type: 'info' });
      const data = await fetchStreamUrl(item, type, type === 'tv' ? selectedSeason : undefined, type === 'tv' ? 1 : undefined, 'moviebox');
      const mp4 = data.links.find(l => l.url.includes('.mp4')) || data.links[0];
      if (!mp4) throw new Error('No downloadable link found');
      let lastProgress = 0;
      const poster = `${IMAGE_BASE_URL}${POSTER_SIZE}${item.poster_path}`;
      const title = item.title || item.name || 'Unknown';
      const res = await downloadVideoToIndexedDB(mp4.url, (received, total) => {
        const pct = total ? Math.floor((received / total) * 100) : Math.min(99, Math.floor(received / (1024 * 1024)));
        if (pct !== lastProgress) {
          lastProgress = pct;
          addDownload({ id: 'temp', originalId: item.id, originalType: type, title, poster, progress: pct });
        }
      }, async () => {
        // Renew signed/proxied URL if current one times out/expires
        const refreshed = await fetchStreamUrl(item, type, type === 'tv' ? selectedSeason : undefined, type === 'tv' ? 1 : undefined, 'moviebox');
        const freshMp4 = refreshed.links.find(l => l.url.includes('.mp4')) || refreshed.links[0];
        if (!freshMp4) throw new Error('No downloadable link found on refresh');
        return freshMp4.url;
      });
      addDownload({ id: res.id, originalId: item.id, originalType: type, title, poster, progress: 100, completed: true, sizeBytes: res.size });
      setToast({ message: 'Downloaded', type: 'success' });
      navigate('/downloads');
    } catch (e: any) {
      console.error(e);
      setToast({ message: e.message || 'Download failed', type: 'error' });
    }
  };
  
  const handleEpisodePlay = (episode: Episode) => {
     // The prefetched URL is only for the first episode of the first available season.
     // Only pass it along if that's the episode being played.
     const firstValidSeasonNumber = item.seasons?.find(s => s.season_number > 0 && s.episode_count > 0)?.season_number ?? 1;
     const isPrefetchedEpisode = episode.episode_number === 1 && selectedSeason === firstValidSeasonNumber;
     navigate('/player', { state: { item, type, season: selectedSeason, episode, streamUrl: isPrefetchedEpisode ? prefetchedStreamUrl : null } });
  }
        ...
        <div className="animate-fade-in-up" style={{animationDelay: '400ms'}}>
            <div className="flex gap-3 my-4">
                ...
                <button onClick={handleDownload} className="w-28 h-12 rounded-lg bg-[var(--surface)] text-[var(--text-light)] shadow-lg text-xs font-bold flex flex-col items-center justify-center btn-press">
                  ...
                  <span className='mt-1'>{t('downloads')}</span>
                </button>
            </div>
        </div>
